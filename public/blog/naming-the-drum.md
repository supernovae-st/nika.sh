...
Nika takes its name from a modern liberation myth: a sun god announced by drums and laughter, turning fear into movement. The figure does not win by building a better cage. The cage becomes absurd, then breaks.

That rhythm fits a workflow engine unusually well.

A workflow is not one grand declaration of independence. It is a small repeatable act. The file stays yours. The engine runs where you choose. The model can change without taking the plan with it. The trace records what actually happened.

Run after run, the promise becomes mechanical.
...
A liberation name on a closed service would be decoration. Here it creates a test.

Can you keep the workflow as plain text? Can you run it through a local engine? Can you inspect the boundaries before execution and the trace after it? Can another implementation read the public specification? Can the system keep working without making a hosted account the source of truth?
