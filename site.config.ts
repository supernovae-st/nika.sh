...
/* the error register's per-code pages · the engine stamps
   docs_url = https://nika.sh/errors/<CODE> on every check finding, and a
   docs_url must land on a real 200 — the SPA fallback serves the content but
   the 404 status kills curl/agents/link-checkers. Mirrors public/errors/
   catalog.json — kept literal (this file stays import-free); the vitest drift
   gate (errors.test.ts) fails when the catalog and this list disagree. */
