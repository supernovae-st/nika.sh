...
export const PLAY_BREAKS: Record<string, PlayBreak> = {
  ...
  'gate-and-act': {
    label: 'Misspell the const key',
    find: 'url: "${{ const.source_url }}"',
    replace: 'url: "${{ const.sourceurl }}"',
    fires: 'NIKA-VAR-001',
    lesson: 'every ${{ const.* }} reference must name a declared entry · typos die at check, not at 3am',
  },
  fanout: {
