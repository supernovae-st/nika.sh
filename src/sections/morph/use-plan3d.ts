...
/* ─── usePlan3D · the capability gate for the 3D DAG layer (wave H) ───────────
   ...
   allowed, a real WebGL context available, AND the morph section has come
   near the viewport once (so the lazy chunk never loads for a visitor who
   stays above the fold). Everywhere else the DOM story stays the one truth. */
