...
export const CHANGELOG: ChangelogEntry[] = [
  {
    date: '2026-08-21',
    tag: 'site',
    title: 'nika.sh v4.23 · the origin becomes one story',
    body: 'The six origin records now read as one continuous arc, from the first objection to the name. The series page replaces its generic card grid with a dated ledger that shows reading order, source receipts and the evidence boundary before a reader opens a chapter.',
    gh: 'https://github.com/supernovae-st/nika.sh/releases/tag/v4.23.0',
  },
  {
    date: '2026-08-21',
    tag: 'site',
  ...
  },
  {
    ...
    body: 'The film taught the grammar; now every plan on the site speaks it. A shared PlanMap carries the film’s flat map verbatim: the slab card with its verb-hued inset, the wave captions in the film’s own words, the hairline chevron on every wire. /use-cases, the home gallery and /play’s live plan all draw the same object the film flattens into. Plans that outgrow their column scroll inside their own well, keyboard included, on every surface. The journal earns its register filter: deep-linkable, counts derived from the posts themselves, issue numbers absolute under any view. Two engine-truth posts land from live proof sessions: kill -9 a run and resume it (finished work never runs twice), and the approval gate as a task: fails closed headless, the human’s answer rides the resume, a refusal is a run that worked. And a long-lived ghost dies on the way out: the hero’s background field could lose its GL init for a whole session and composite blank; a watchdog now remounts it until the first frame paints, on the site and in the golden harness both.',
