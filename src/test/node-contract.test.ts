...
describe('la carte projetée · une géométrie, trois surfaces', () => {
  ...
  it('elle ne porte aucune couleur QU’UN HÔTE NE PUISSE REMPLACER', () => {
    /* La règle d'origine interdisait tout hex, et c'était juste tant que la
       feuille ne portait que de la géométrie. Les rendus de valeur sont de la
       couleur DÉLIBÉRÉMENT — un chemin porte le vert du succès — donc la règle
       se précise plutôt que de céder : un hex est permis UNIQUEMENT en repli
       d'un var(), parce que là il est remplaçable par construction. Une couleur
       qu'aucun jeton ne couvre reste interdite. */
    const bound = /var\(--[a-z0-9-]+,\s*(#[0-9a-f]{3,8}|rgba?\([^)]*\))\s*\)/gi
    const stripped = NODE_CSS.replace(bound, 'var(--bound)')
    const hex = stripped.match(/#[0-9a-f]{3,8}\b/gi) ?? []
    const rgb = stripped.match(/\brgba?\(/g) ?? []
    expect([...hex, ...rgb],
      'une couleur en dur, hors repli de var() : aucune surface ne pourrait la reprendre')
      .toEqual([])
    /* et la clause qui empêche la règle de devenir creuse : il DOIT rester des
       valeurs sémantiques branchables, sinon le gate passerait sur une feuille
       qui a simplement cessé d'en porter */
    expect((NODE_CSS.match(/var\(--nk-ess-/g) ?? []).length,
      'plus aucun rendu de valeur branchable · la sémantique a disparu').toBeGreaterThan(4)
  })
