...
if (isMain) {
  ...
  /* The route graph needs the compact identity of every post, but the prose
     used by cards and article heads does not belong in the initial bundle.
     Keep that copy beside the already-lazy bodies and serve it through the
     same SSR island recipe. Ten catch-up essays otherwise added 1.8KB gz to
     every route, including pages that never render the journal. */
  const metas = posts.map(({
    tokens: _tokens,
  ...
  const copy = Object.fromEntries(posts.map((p) => [p.slug, {
...
/* Card + article-head prose shares the bodies' register-diet lane. Pages
   ...
   navigation. */
export const BLOG_POST_COPY: Record<string, BlogPostCopy> = ${JSON.stringify(copy, null, 2)}
`
