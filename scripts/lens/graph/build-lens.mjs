/* ─── build-lens · the language-graph compiler (WO-2 of the lens plan) ─────
   ONE compiler over the descriptor table (scripts/lens/graph/sets.yaml — the one
   AUTHORED input) plus the gated sources census.mjs already proved. It emits
   the lens surfaces; nothing downstream re-declares a set, a count, a rail
   or a redirect.

   Outputs (each idempotent · vitest recompiles + byte-diffs):
     1  src/content/lens.generated.ts        nodes + edges (lazy consumers)
     1b src/content/lens-meta.generated.ts   provenance + counts (chrome-safe)
...
export const LENS_NODES: LensNode[] = ${JSON.stringify(nodes, null, 2)}
...
${
  ...
  chunk(uniqEdges, 400)
    .map(
      (part, i) =>
        `const LENS_EDGES_${i}: LensEdge[] = ${JSON.stringify(part, null, 2)}`,
    )
    .join('\n\n')
}

export const LENS_EDGES: LensEdge[] = [
${chunk(uniqEdges, 400)
  .map((_, i) => `  ...LENS_EDGES_${i},`)
  .join('\n')}
]

export const LENS_INDEX: Record<string, LensNode> = Object.fromEntries(
  LENS_NODES.map((n) => [n.id, n]),
)
`,
)
...
export const LENS_HUBS = ${JSON.stringify(
    S.sets.layers.map((l) => ({ id: l.id, title: l.title, hub: l.hub, exists: l.hub_exists, lands: l.lands ?? null })),
    null,
    2,
  )} as const

export const LENS_SCORE = ${JSON.stringify({ score, waived, unarmed }, null, 2)} as const
...
/* ─── the design graph (fenêtre A · LENS_DESIGN_GRAPH §1) ────────────────────
   ...
   byte-diffed by lens.test; the measured forks consume them and the
...
const designCss = `/* design.generated.css — AUTO-GENERATED by scripts/lens/graph/build-lens.mjs from
   scripts/lens/graph/sets.yaml design: (site facts) + the spec spine (verb/mark
   tokens · values change SPEC-FIRST). DO NOT EDIT · regenerate:
   node scripts/lens/graph/build-lens.mjs · gate: lens.test byte-diff +
...
export type LensLayerId = keyof typeof LAYER_HEX

/** normalized 0..1 triples — the three.js seam, extended to layers */
export const LAYER_RGB: Record<LensLayerId, readonly [number, number, number]> = {
...
export type LensKind = keyof typeof KIND_HEX
...
export const KIND_OF_SET: Record<string, LensKind> = ${JSON.stringify(kindOfSet, null, 2)}

/** normalized 0..1 triples — the three.js seam, extended to kinds */
export const KIND_RGB: Record<LensKind, readonly [number, number, number]> = {
...
const designPalette = JSON.stringify(
  {
    design_palette: 1,
    _generated: 'scripts/lens/graph/build-lens.mjs — DO NOT EDIT (sets.yaml design: + the spec spine)',
...
const dtcg = JSON.stringify(
  {
    $description:
      'nika.sh design tokens — AUTO-GENERATED by scripts/lens/graph/build-lens.mjs from sets.yaml design: + the nika-spec spine. License: Apache-2.0 (machine artifact).',
...
const forcedColorsCss = `/* design.forced-colors.css — AUTO-GENERATED by scripts/lens/graph/build-lens.mjs
   (sets.yaml design: · §7b). DO NOT EDIT · regenerate: node scripts/lens/graph/build-lens.mjs */
...
const vocabTs = GEN(
  ...
  `/** the market-language bridge (scripts/lens/graph/market-vocab.yaml · authored ·
...
/* the blog cross-rails ride their OWN module (the register-diet law: this
   ...
   island. A static import outside the doors goes red in lens.test). */
...
/* LENS_PATHS (output 3 · §7 WO-4): the routes the lens introduced, written
...
const lensPaths = [
  ...S.sets.surfaces.filter((x) => x.lens_route && x.exists).map((x) => x.url),
  ...S.sets.layers.filter((l) => l.lens_route && l.hub_exists).map((l) => l.hub),
...
if (a === -1 || b === -1) throw new Error('site.config.ts: LENS PATHS markers missing')
const nextSiteConfig =
  siteConfig.slice(0, a) + MARK_A + JSON.stringify(lensPaths).replaceAll('","', "', '").replace('["', "['").replace('"]', "']") + siteConfig.slice(b)

/* hub data (the lens-born hubs' render module · chrome-lean: pages never
...
const hubData = S.sets.layers
  .filter((l) => l.lens_route && l.hub_exists)
  .map((l) => ({
    id: l.id,
    title: l.title,
    hub: l.hub,
...
/* ─── write + report ───────────────────────────────────────────────────────*/
if (!REPORT_ONLY) {
  writeFileSync(join(ROOT, 'src/content/lens.generated.ts'), lensTs)
  writeFileSync(join(ROOT, 'src/content/lens-meta.generated.ts'), metaTs)
  ...
  writeFileSync(
    ...
    JSON.stringify({ lexicon: 1, derived_from: 'build-lens (the language graph)', entries: untranslatables }, null, 2) + '\n',
  )
  writeFileSync(join(ROOT, 'public/redirects.json'), redirectsJson)
}

const memberCount = nodes.filter((n) => n.kind === 'member').length
console.log(`lens score · ${score}`)
...
if (deductions.length) {
  ...
  console.log(`lens: RED (${score})`)
  process.exit(2)
}
console.log('lens: GREEN (100)')
