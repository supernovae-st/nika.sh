...
for (const w of WORDS) {
  ...
  if (w.verb) {
    ...
    const codes = errors.filter((e) => re.test(e.failure)).map((e) => e.code)
    entries.push({ word, templates: inTemplates, codes })
    continue
  }
  ...
  const inTemplates = templates.filter((t) => keyRe(word).test(t.yaml)).map((t) => t.name)
  /* codes whose registered failure line NAMES the word (mechanical) */
  const re = new RegExp(`(^|[^a-z0-9_])${word}([^a-z0-9_]|$)`, 'i')
  const codes = errors.filter((e) => re.test(e.failure)).map((e) => e.code)

  entries.push({ word, ...(usage ? { usage } : {}), templates: inTemplates, codes })
}
