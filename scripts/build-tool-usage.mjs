...
/* the refs twin · the cross-reference fields ONLY (no yaml slices). graph.ts
   inverts these sync in the initial bundle while the full registry (the
   heavy verbatim excerpts) rides its own chunk behind the room's island.
   Same run, same entries — the twin cannot disagree with the registry. */
const refsBanner = `// tool-usage-refs.generated.ts — AUTO-GENERATED by scripts/build-tool-usage.mjs
...
const OUT_REFS = join(ROOT, 'src', 'content', 'tool-usage-refs.generated.ts')
writeFileSync(
  OUT_REFS,
  `${refsBanner}${JSON.stringify(Object.fromEntries(entries.map((e) => [e.bare, { templates: e.templates, errorCodes: e.errorCodes }])), null, 2)}\n`,
)
console.log(`tool-usage → ${entries.length} rooms (${entries.filter((e) => e.source.kind === 'template').length} skeleton excerpts · ${entries.filter((e) => e.source.kind === 'crafted').length} crafted files) + refs twin`)
