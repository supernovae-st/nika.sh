 ...
 *   public/brand/icons/<ns>-<name>.svg   the served catalog (lucide ISC + house)
 ...
 *   · every lucide glyph exists in lucide-static; every local file exists
...
function resolveGlyph(id, e) {
  ...
  const p = join(LUCIDE, `${e.lucide}.svg`)
  if (!existsSync(p)) return fail(`${id}: lucide glyph missing: ${e.lucide}`)
  const raw = readFileSync(p, 'utf8')
  return { viewBox: '0 0 24 24', body: innerOf(raw), mode: 'stroke', strokeWidth: 2, set: 'lucide' }
}
...
for (const id of ids) {
  const e = entities[id]
  const g = resolved[id]
  const credit = g.set === 'lucide' ? `glyph: Lucide (ISC) · ${e.lucide}` : 'glyph: house artwork'
  const strokeAttrs =
    g.mode === 'stroke'
  ...
  outputs.set(
    ...
    `<!-- nika icon library · ${id} · ${credit} · generated by design/build.mjs -->\n` +
      `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${g.viewBox}"${strokeAttrs} data-nika="${id}" aria-hidden="true">${g.body}</svg>\n`,
  )
}
      ...
      entities: Object.fromEntries(
        ids.map((id) => [
          id,
          {
            ...
            svg: `https://nika.sh/brand/icons/${fileId(id)}.svg`,
            set: resolved[id].set,
            lucide: entities[id].lucide ?? null,
            central: entities[id].central ?? null,
...
const CLASS = { verb: 'Verb', builtin: 'Builtin', feature: 'Feature', state: 'RunState', ui: 'UiGlyph', social: 'SocialMark' }
...
for (const id of ids) {
  ...
  if (e.lucide) lines.push(`  nkd:lucideGlyph ${lit(e.lucide)}`)
  if (e.central) lines.push(`  nkd:centralGlyph ${lit(e.central)}`)
  ...
  lines.push(`  nkd:servedAt <https://nika.sh/brand/icons/${fileId(id)}.svg>`)
  ttl += '\n' + lines.join(' ;\n') + ' .\n'
}
