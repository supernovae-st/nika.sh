...
An llm pipeline is a graph of model calls, tools and processes, wired by data dependencies. That is the whole definition. And the industry's default answer to "how do I build one" is a framework: pick an SDK, learn its abstractions, write the glue code that calls the model, passes the output, handles the retry — a program that *performs* the graph, step by imperative step.

But look at what a pipeline actually *is* for a second. The model call does not care what called it. The file write does not care what wrote the content. The only real structure is which task needs whose output — and that is not behavior, that is a **shape**. Shapes are declared, not programmed.
